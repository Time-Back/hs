...
router.get("/verify_uname", (req, res) => {
  ...
  if (!$uname) {
    ...
    if ($uname.length >= 3 && $uname.length <= 12) {
      var sql = "select * from hs_user where uname=?";
...
//1.2注册
//验证手机号码是否存在
router.get("/verify_phone", (req, res) => {
  var $phone = req.query.phone;
  var sql = "select * from hs_user where phone=?";
...
router.post("/reg", (req, res) => {
  ...
  var sql =
    "insert into hs_user (uname,upwd,email,phone,gender,reg_time) values (?,?,?,?,?,?) ";
  pool.query(
    sql,
    [$uname, $upwd, $email, $phone, $gender, $reg_time],
    (err, result) => {
      if (err) throw err;
...
router.get("/login", (req, res) => {
  ...
  if ($uname && $upwd) {
    var sql = "select * from hs_user where uname=? and upwd=?";
...
router.post("/update", (req, res) => {
  ...
  var sql =
    "update hs_user set uname=?,upwd=?,email=?,phone=?,gender=? where uid=?";
  pool.query(
    sql,
